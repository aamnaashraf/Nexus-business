...
import { MessageCircle, Building2, UserCircle, BarChart3, Briefcase, Globe, Linkedin, Twitter } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="sm:flex sm:space-x-6">
            ...
            <div className="mt-4 sm:mt-0 text-center sm:text-left">
              ...
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start mt-3">
                {ip?.ticketSize && (
                  <Badge variant="accent">{ip.ticketSize}</Badge>
                )}
                {ip?.investmentFocus?.map((focus, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Investment Focus */}
          {ip && (ip.investmentFocus?.length > 0 || ip.ticketSize) && (
            <Card>
              <CardHeader>
              ...
              <CardBody>
                <div className="space-y-4">
                  {ip.investmentFocus && ip.investmentFocus.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {ip.investmentFocus.map((focus, i) => (
                          <Badge key={i} variant="primary">{focus}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
                  {ip.ticketSize && (
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 mb-1">Ticket Size</h3>
                      <p className="text-gray-900 font-medium">{ip.ticketSize}</p>
                    </div>
                  )}
                  {ip.portfolioCompanies > 0 && (
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 mb-1">Portfolio Companies</h3>
                      <p className="text-gray-900 font-medium">{ip.portfolioCompanies}</p>
                    </div>
                  )}
                </div>
              </CardBody>
                  ...
                  {investor.investmentHistories.map((h) => (
                    <div key={h.id} className="border-l-2 border-primary-200 pl-4">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {new Date(h.investmentDate).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'short',
                        })}
                      </p>
