...
import { ChatMessage as ChatMessageType } from '../../services/messageAPI';
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, isCurrentUser }) => {
  ...
  return (
    <div className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'} mb-4 animate-fade-in`}>
      {!isCurrentUser && (
        <Avatar src={avatarSrc} alt={senderName} size="sm" className="mr-2 self-end" />
      )}
      ...
      {isCurrentUser && (
        <Avatar src={avatarSrc} alt={senderName} size="sm" className="ml-2 self-end" />
      )}
    </div>
  );
};
